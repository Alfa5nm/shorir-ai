...
def set_font(run, size=11, bold=False, color=INK, name="Aptos"):
    run.font.name = name
...
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
set_font(p.add_run("SHORIR AI"), 32, True, INK, "Aptos Display")
p.paragraph_format.space_after = Pt(8)
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
set_font(p.add_run("Bangla-First Privacy-Aware Fitness Intelligence"), 18, False, MUTED, "Aptos Display")
...
set_font(p3.add_run("25 June 2026"), 10.5, False, MUTED)
doc.add_page_break()

add_heading(doc, "Executive Summary", 1)
add_body(doc, "SHORIR AI is a Bangla-first fitness companion that combines browser-side pose analysis, a bilingual exercise library, personalized workout planning, Bangladeshi diet guidance, image-based calorie review, and progress diagnostics in one coordinated product.")
...
add_figure(doc, ROOT / "tmp/home-ui.png", "Figure 1. Coordinated dashboard with light theme and competition branding.")
...
add_figure(doc, ROOT / "tmp/coach-ui.png", "Figure 2. Live squat coach with activity stage, movement guide, and strict analyzer controls.")
...
add_table(doc, ["Deliverable", "Location"], [
    ("Live website", "Recorded in deliverables/SUBMISSION.md after deployment."),
